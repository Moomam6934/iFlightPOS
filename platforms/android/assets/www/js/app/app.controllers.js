...
.controller('PaymentCtrl', function($scope, $ionicModal, $state, $ionicPopup, PaymentService, $stateParams, ShopService, $filter, MasterService, $ionicActionSheet) {
    ...
    $scope.curren = function(currency) {
        ...
        if ($scope.iFlightData.payments[0].id == $scope.itemTypePay.id && $scope.iFlightData.payments.length == 1) {
            $scope.itemTypePay.currency.money = $scope.iFlightData.total_gross_amount / currency.exchange;
        } else if ($scope.iFlightData.payments[0].id == $scope.itemTypePay.id && $scope.iFlightData.payments.length > 1) {
            $scope.calculatorTotal = $scope.itemTypePay.amount;
            $scope.itemTypePay.currency.money = $scope.calculatorTotal / currency.exchange
        } else {
            if (a == 0) {
                $scope.itemTypePay.currency.money = ($scope.iFlightData.total_gross_amount - $scope.iFlightData.sold_total) / currency.exchange;
                a = $scope.itemTypePay.currency.money;
            } else {
                $scope.itemTypePay.currency.money = a / currency.exchange;
            }
    ...
    $scope.$watch('payments', function(n, o) {
        ...
        if (result > $scope.iFlightData.total_gross_amount) {
            ...
            //     $scope.itemTypePay.currency.money = $scope.calculatorTotal / $scope.itemTypePay.currency.currencys.exchange;
            // }
            $scope.changemoney = 0;
            // $scope.curren();
        }
